(function() {
  ...
  module.exports = {
    ...
    saveJade: function saveJade(name, cb) {
      ...
      var originalExt = path.extname(_this.filePath).substring(1);
      var originalName = path.basename(_this.filePath, originalExt);
      originalName = originalName.substring(0, originalName.length - 1);
      var originalPath = path.dirname(_this.filePath)
      var commentName = name.replace('$1', originalName).replace('$2', originalExt);
      var result = path.resolve(originalPath, commentName);

      fs.writeFile(result, _this.jadeCode, function writeFile(err) {
        if (err) {
